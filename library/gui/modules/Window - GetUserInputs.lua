...
local function return_values(apply, func)
    ...
    if apply then
        ...
        local vals = {}
        for i = 1, GUI.Elements.UserInputs_wnd.num_inputs do
...
local function wnd_open(self)
    ...
    GUI.Elements.UserInputs_ok.x = self.x + (self.w / 2) - 72
    GUI.Elements.UserInputs_cancel.x = self.x + (self.w / 2) + 8
...
local function txt_enter(self)
    ...
    GUI.Elements.UserInputs_ok:exec()
...
function GUI.GetUserInputs(title, captions, defaults, ret_func, extra_width)
    ...
    GUI.Elements.UserInputs_wnd.onopen = wnd_open

    GUI.Elements.UserInputs_wnd.close = wnd_close

    GUI.newfocus = GUI.Elements.UserInputs_txt_1
    ...
    GUI.Elements.UserInputs_ok.func = function() GUI.Elements.UserInputs_wnd:close(true) end
    GUI.Elements.UserInputs_cancel.func = function() GUI.Elements.UserInputs_wnd:close() end


    GUI.Elements.UserInputs_wnd:open()
